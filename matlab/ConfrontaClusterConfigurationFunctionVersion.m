function [] = ConfrontaClusterConfigurationFunctionVersion( suffix, alpha_par, iter_par, clust_method, ker_name )
%funzione che prende le clustering configuration dei grafi originali, le
%unisce a quelle samplate, le sorta e salva il risultato in un file csv
...
load_file = strcat('ClustAndConnManagerWORKSPACE-', file_name_suffix, '.mat');
load(load_file);
...
main_directory = strcat('D:\Dropbox\GenerativeModelsOfGraphs\OUTPUTS\Run scuole approccio nuovo\', clust_method);
path = (strcat(main_directory, '\', ker_name, '_subpar=', num2str(subpar), '_clustpar=', num2str(clustpar), '\ConfrClustConf_Alpha=', num2str(alpha), '_Iter=', num2str(iterations)));
mkdir(path);

pause(1);

dlmwrite(strcat(path, '\ConfrClustConf_', 'A=', num2str(alpha), '_Iter=', num2str(iterations), '-', file_name_suffix, '.csv'), clust_conf_sorted, 'delimiter', ',');
