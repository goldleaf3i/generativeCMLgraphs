...
function [Ggen, FgenLin, maxNodes, feasibleEdgeVector, feasibilityEdgeVector, selectedEdgesIndexedByFeasibleEdgeVector, selectedEdgeIndexedByType, P, dimBlocked] = initialConfigurationNodesNoLArgmax3(Cgen, Fgen, numFgen, kernelDistanceFunction, kernelParameter, alfa, beta, gamma, gammaCut, zeta, iota, edgeExistenceThreshold, distances, distancesCut, cutDegreeSum, connectedToGen, compatibleNodesGen, connectedToInternal, compatibleNodesInternal, corridors_label,numLabel,cont,skip_errors)
    ...
    for ki=1:numF
        ...
        for kj=ki+1:numF
            ...
            for n=1:numE
                if feasibilityEdgeVector{ki,kj}(n) > 0
                    ...
                    for l=1:size(nodePairs,1)
                        ...
                        if ( (mandatoryConnections(ki) && (~ismember(labeli,mandatoryLabels))) || (mandatoryConnections(kj) && (~ismember(labelj,mandatoryLabels))))
                            ...
                            if PinducedSubgraphij == 0 
                                warning(strcat('Check potrebbe esserci errore al grafo  ',num2str(cont),' ',datestr(datetime)));
                                dimij
                            end
                        end
            ...
            if Cgen(ki,kj) > 0 && sum(feasibilityEdgeVector{ki,kj}) > 0
                ...
                if probabilities(maxP) == 0
                    disp('VAFFANCULO');
                    warning(strcat('errore al grafo ',num2str(cont),' ',datestr(datetime)));
                end
                if sumP > 0
                    probIndex = maxP;
