...
%INPUT:  Cgen - matrice di connettività tra i sottografi di Fgen
...
function [Ggen, FgenLin, maxNodes, feasibleEdgeVector, feasibilityEdgeVector, selectedEdgesIndexedByFeasibleEdgeVector, selectedEdgeIndexedByType, P, dimBlocked] = initialConfigurationNodes3(Cgen, Fgen, numFgen, kernelDistanceFunction, kernelParameter, alfa, beta, gamma, gammaCut, zeta, iota, edgeExistenceThreshold, distances, distancesCut, cutDegreeSum, connectedToGen, compatibleNodesGen, connectedToInternal, compatibleNodesInternal, corridors_label,numLabel,cont,skip_errors)
    ...
    %uno o più corridoi (label={1,2,13})
    ...
    %calcolo le probabilità dell'esistenza degli edge per ogni coppia di sottografi
    for ki=1:numF
        Fi = FgenLin{ki};
        for kj=ki+1:numF
            Fj = FgenLin{kj};
            for n=1:numE
                if feasibilityEdgeVector{ki,kj}(n) > 0
                    ...
                    for l=1:size(nodePairs,1)
                        ...
                        Pdegi = degi/cutDegreeSum;
                        Pdegj = degj/cutDegreeSum;
                        
                        Pexistcutij = iota(labeli,labelj);
                                                
                        %if Pexistcutij < edgeExistenceThreshold || Pdegi == 0 || Pdegj == 0 || ((mandatoryConnections(ki)==1 && mandatoryConnections(kj)==1) && (~ismember(labeli,mandatoryLabels) || ~ismember(labelj,mandatoryLabels)))
                        if Pexistcutij < edgeExistenceThreshold || Pdegi == 0 || Pdegj == 0 ...
                            ...
                            dimij = size(distancesCut,1) + 1;
                            distanceMatrix = zeros(dimij);
                            for i=1:dimij-1
                                for j=i:dimij-1
                                    distanceMatrix(i,j) = distancesCut(i,j);
                                    distanceMatrix(j,i) = distanceMatrix(i,j);
                                end 
                            end
                            for j=1:dimij
                                distanceMatrix(dimij,j) = kernelDistanceFunction(inducedSubgraphij,gammaCut{j}, kernelParameter);
                        ...
                        P{n,ki,kj,l} = PinducedSubgraphij*Pexistcutij*Pdegi*Pdegj;
                    end
                end
            end
            ...
            if Cgen(ki,kj) > 0 && sum(feasibilityEdgeVector{ki,kj}) > 0
                ...
                if sumP > 0
                    ...
                    %se le probabilità di aggiunta edge sono tutte zero
                    %prendo un edge con probabilità uniforme
                    if skip_errors
