%%
% COME BLOCKED SAMPLING: SE NON TROVA DEI MATCH DENTRO I NODI GIA VISTI (QUINDI DENTRO LA PARTE DEL GRAFO GIA VISTA, E' UN BUG) IGNORA LE CONNESSIONI GIA VISTE. 
% DOVREBBE FUNZIONARE COMUQUE PERCHE A} LA PARTE DEL BLOCKED SAMPLING A LIVELLO PRECEDENTE é MANTENUTA B} CONNETTO I CORRIDOI E ALLA FINE CE NE é SOLO UNO
%
...
%try 
    ...
    save(strcat('Predict/matfiles/ERRORE',num2str(numeroerrori)),'ME');
    disp([' configurazione con algoritmo: ', num2str(sampling_algo), ' con labels: ', num2str(sampling_with_labels), ' e argMAX: ', num2str(isARGMAX)]);
    if isARGMAX
        switch sampling_algo
            case 1
